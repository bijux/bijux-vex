...
def validate_index_meta(
    ...
    *,
    ...
    if settings and settings.m is not None and as_int(params.get("M"), 0) != int(
        settings.m
    ):
        raise InvariantError(message="ANN index M parameter mismatch")
    if settings and settings.ef_construction is not None and as_int(
        params.get("ef_construction"), 0
    ) != int(settings.ef_construction):
        raise InvariantError(message="ANN index ef_construction mismatch")
    if settings and settings.ef_search is not None and as_int(
        params.get("ef_search"), 0
    ) != int(settings.ef_search):
        raise InvariantError(message="ANN index ef_search mismatch")
    if settings and settings.max_ef_search is not None and as_int(
        params.get("ef_search"), 0
    ) > int(settings.max_ef_search):
